...
def merge_impact_mapping(
    ...
    """
    Merge impact mapping data with Gleason extraction results.

    Args:
        df_gleason (pd.DataFrame): Combined Gleason dataframe.
        df_impact_mapping (pd.DataFrame): DataFrame with SAMPLE_ID and SOURCE_ACCESSION_NUMBER_0.
        col_accession (str): Column name for accession number in df_gleason.
...
def main(argv=None) -> int:
    ...
    table_gleason_epic = get_extracted_table(config, 'pathology_gleason_calls_epic')
