""""
pathology_extract_dop_epic.py

This script will extract dates of procedures that are buried in the part description column
...
class PathologyExtractDOPEpic(object):
    def __init__(
            self,
            fname_minio_env,
            fname,
    ...
    def _compute_dop(self, df):
        ...
        regex_str = r'DOP:[ ]*(\d{1,2}/\d{1,2}/\d{2,4}|\d{4}-\d{2}-\d{2})'
        ...
        regex_str = r'[ ]*(\d{1,2}/\d{1,2}/\d{2,4}|\d{4}-\d{2}-\d{2})'
    ...
    def _extract_date_of_procedure(
            self,
            df,
            regex_str,
            col_spec_sub,
